...
class ExporterService {
     ...
     */
    public static function ExportToCsv($someArray) {
        $fileName = uniqid() . ".csv";
     ...
     */
    private static function ExportToXls($someArray) {
        $fileName = uniqid() . ".xls";
        $filePath = ExporterService::settingsExportPath . $fileName;
     ...
     */
    private static function ExportToXlsx($someArray) {
    ...
    public static function ExportJsonToFile($json, $exportType) {
        ...
        switch ($exportType) {
            case ExportTypeEnum.XLSX:
                $fileName = ExporterService::ExportToXlsx($someArray);
                break;
            case ExportTypeEnum.XLS:
                $fileName = ExporterService::ExportToXls($someArray);
                break;
            case ExportTypeEnum.CSV:
                $fileName = ExporterService::ExportToCsv($someArray);
                break;
        }
